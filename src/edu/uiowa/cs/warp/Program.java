...
public class Program implements SystemAttributes {
	...
	// private ScheduleChoices schedulerSelected; // set the type of scheduler
	// selected
	/**
	...
	private void initializeClassAttributes() {
		/*
		 * requires workLoad to have already been set. If not, all class attributes will
		 * be null!
		 */
		if (workLoad != null) {
			this.warpOptions = workLoad.getOptions();
			scheduleBuilt = null;
			setScheduleSelected(warpOptions.getSchedulerSelected());
			realTimeHARTflag = false;
			optimizationRequested = true;
			nChannels = warpOptions.getNumChannels();
			verbose = warpOptions.verboseMode();
			channelsAvailable = new Channels(nChannels, verbose);
			// this.reportLatency = warpOptions.latencyRequested();
			deadlineMisses = new Description();
	...
	/**
	 * Dispatches to the correct synthesis path based on the selected scheduler If
	 * {@link #workLoad} is {@code null}, this method returns without building.
	 */
	private void buildProgram() {
		 ...
		 */
		switch (warpOptions.getSchedulerSelected()) { // select the requested scheduler
...
//      }
			if (optimizationRequested) { // only specify when optimization not requested
...
//      }
	...
	/**
	 * Extracts the first {@code #<channel>} value from a formatted instruction
	 * string.
	 * 
	...
	private String findNextAvailableChannel(ProgramSchedule schedule, String nodeName, Integer currentTime,
		...
		// get the prior schedule time slot to see what channels were used in that slot
		if (currentTime != null && currentTime > 0) { 
			Integer priorTime = currentTime - 1;
			
			// get a copy of the prior time slot from src and snk
			priorInstructionTimeSlot = schedule.get(priorTime); 
			...
			//Call helper function to remove unavailable channels based on src and snk parameters.
			removeChannelFromParam(dsl.getInstructionParameters(srcPriorInstruction), channels);

			removeChannelFromParam(dsl.getInstructionParameters(snkPriorInstruction), channels);
			
		}
		// get the last channel from the node and increment the channel to avoid using the same channel.
		Integer channel = workLoad.getNodeChannel(nodeName); 
		channel++; 
		
		// reset when channel hits MAX
		...
		while (!channelFound && !channels.isEmpty()) { 
			...
			if (channelRemoved) {
				// newChannel has the name of the channel as a string
				newChannel = channelString;
				channelFound = true;
			} else {
				// try another channel
				channel += 1;
				// reset when channel has reached MAX
				if (channel >= getNumChannels()) { 
					channel = 0;
				}
			}
		}
		// returns UNKNOWN to indicate no channel found.
		return newChannel; 
	...
	/**
	 * Returns the synthesized schedule. May be {@code null} if building failed or
	 * was skipped.
	 * 
